"""Generate a grouped bar chart for PMV/PPD across wall temperatures.

Run from the project root:
    python3 scripts/make_pmv_ppd_diff_wall_temperatures_figure.py
"""
...
DATA_FILE = Path("data_files/pmv_ppd_diff_wall_temperatures/average_pmv_ppd.csv")
OUTPUT_DIR = Path("figures")
OUTPUT_FILE = OUTPUT_DIR / "pmv_ppd_diff_wall_temperatures.png"
...
def load_data() -> pd.DataFrame:
    ...
    data = data.sort_values("Case Wall Temperature (K)").reset_index(drop=True)
    data["Wall temperature (°C)"] = data["Case Wall Temperature (K)"] - 273.15
    return data
