"""Generate a clean PMV/PPD grouped bar chart by diffuser offset.

Run from the project root:
    python3 scripts/make_pmv_ppd_diff_diffuser_offset_figure.py
"""
...
DATA_FILE = Path("data_files/pmv_ppd_diff_diffuser_offset/average_pmv_ppd_offset.csv")
OUTPUT_DIR = Path("figures")
OUTPUT_FILE = OUTPUT_DIR / "pmv_ppd_diff_diffuser_offset.png"
...
def load_data() -> pd.DataFrame:
    return pd.read_csv(DATA_FILE).sort_values("Diffuser offset (cm)").reset_index(drop=True)
...
def configure_axis(axis: plt.Axes, data: pd.DataFrame, ylabel: str, ylim: tuple[float, float]) -> None:
    ...
    axis.set_xticklabels([f"{value:g}" for value in data["Diffuser offset (cm)"]])
...
def main() -> None:
    ...
    axes[1].set_xlabel("Diffuser offset (cm)")
