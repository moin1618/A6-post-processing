"""Generate a clean PMV/PPD grouped bar chart by inlet velocity.

Run from the project root:
    python3 scripts/make_pmv_ppd_extension_velocity_figure.py
"""
...
DATA_FILE = Path("data_files/pmv_ppd_extension_velocity/average_pmv_pdd.csv")
OUTPUT_DIR = Path("figures")
OUTPUT_FILE = OUTPUT_DIR / "pmv_ppd_extension_velocity.png"
...
def load_data() -> pd.DataFrame:
    return pd.read_csv(DATA_FILE, na_values=["N/A", ""]).sort_values("Inlet Velocity (m/s)").reset_index(drop=True)
...
def configure_axis(axis: plt.Axes, data: pd.DataFrame, ylabel: str, ylim: tuple[float, float]) -> None:
    ...
    axis.set_xticklabels([f"{value:g}" for value in data["Inlet Velocity (m/s)"]])
...
def main() -> None:
    ...
    axes[1].set_xlabel("Inlet velocity (m/s)")
