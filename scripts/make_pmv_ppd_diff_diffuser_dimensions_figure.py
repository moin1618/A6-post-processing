"""Generate a clean PMV/PPD grouped bar chart by diffuser width.

Run from the project root:
    python3 scripts/make_pmv_ppd_diff_diffuser_dimensions_figure.py
"""
...
DATA_FILE = Path("data_files/pmv_ppd_diff_diffuser_dimensions/combined_average_pmv_ppd_results.csv")
OUTPUT_DIR = Path("figures")
OUTPUT_FILE = OUTPUT_DIR / "pmv_ppd_diff_diffuser_dimensions.png"

TEXT_COLOR = "#111111"
AXIS_COLOR = "#222222"
GRID_COLOR = "#E5E7EB"
EXCLUDED_FACE = "#F3F4F6"
CATEGORY_STYLES = {
...
def load_data() -> pd.DataFrame:
    return pd.read_csv(DATA_FILE, na_values=["N/A", ""]).sort_values("w_diffuser (mm)").reset_index(drop=True)


def style_axis(axis: plt.Axes) -> None:
    axis.set_facecolor("white")
    axis.grid(axis="y", color=GRID_COLOR, linewidth=0.65, zorder=1)
    axis.tick_params(
        axis="both",
        which="major",
        direction="in",
...
def add_excluded_region(axis: plt.Axes, data: pd.DataFrame) -> None:
    missing = data[POSTURES[0]["pmv_column"]].isna().to_numpy()
    if not missing.any():
        return

    missing_positions = np.flatnonzero(missing)
    left = missing_positions[0] - 0.5
    right = missing_positions[-1] + 0.5
    axis.axvspan(left, right, facecolor=EXCLUDED_FACE, edgecolor="none", zorder=0.1)
...
def add_bars(axis: plt.Axes, data: pd.DataFrame, column_key: str) -> None:
    ...
    for posture in POSTURES:
        ...
        bar_positions = x_positions + posture["offset"]
        axis.bar(
            bar_positions[valid],
            values[valid],
            width=BAR_WIDTH,
...
def configure_axis(axis: plt.Axes, data: pd.DataFrame, ylabel: str, ylim: tuple[float, float]) -> None:
    ...
    axis.set_xticklabels([f"{value:.0f}" for value in data["w_diffuser (mm)"]])
...
def add_legend(axis: plt.Axes) -> None:
    handles = [
        ...
        Patch(facecolor=EXCLUDED_FACE, edgecolor="none", label="Outside PMV validity range"),
    ]
    axis.legend(
        handles=handles,
        loc="upper center",
...
def main() -> None:
    ...
    configure_axis(axes[0], data, "PMV (unitless)", (0.0, 0.86))
    add_excluded_region(axes[0], data)
    add_bars(axes[0], data, "pmv_column")
    axes[0].yaxis.set_major_locator(FixedLocator([0.0, 0.2, 0.4, 0.6, 0.8]))
    ...
    configure_axis(axes[1], data, "PPD (%)", (0.0, 18.0))
    add_excluded_region(axes[1], data)
    add_bars(axes[1], data, "ppd_column")
    axes[1].set_xlabel("Diffuser width (mm)")
